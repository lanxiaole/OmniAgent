...
from agent_core.config.settings import SYSTEM_DIRS, WORKSPACE_DIR
...
def _safe_path(path: str) -> dict:
    ...
    对于相对路径，如果 CWD 下不存在但 WORKSPACE_DIR 下存在，则自动使用 WORKSPACE_DIR 下的路径。
    这样 Agent 读取 workspace 内的文件（如 uploads/ 下的上传文件）时无需知道完整路径。
    ...
    try:
        ...
        # 如果是相对路径，优先尝试 WORKSPACE_DIR（用于 uploads/ 等工作区文件）
        if not os.path.isabs(expanded) and not expanded.startswith("~"):
            cwd_path = os.path.abspath(expanded)
            workspace_path = os.path.join(WORKSPACE_DIR, expanded)
            # CWD 下存在则用 CWD（保持对项目源码文件的兼容）
            # CWD 下不存在但 WORKSPACE_DIR 下存在，则用 WORKSPACE_DIR
            if not os.path.exists(cwd_path) and os.path.exists(workspace_path):
